...
EXAMPLE_3_OUTPUT = """00000000: E800000000 call 0x00000000
...
0000000E: E800000000 call 0x00000000
00000013: 5A pop edx
00000014: 8D35CB000000 lea 0x000000cb
0000001A: 01D6 add esi,edx
0000001C: 81C6EDFFFFFF add esi,eax,,0xFFFFFFED
00000022: 8D0DED000000 lea 0x000000ed
00000028: 01D1 add ecx,edx
0000002A: 81C1EDFFFFFF add ecx,eax,,0xFFFFFFED
00000030: 8931 mov ecx,ecx
00000032: B906000000 mov ecx,0x00000006
00000037: 51 push ecx
00000038: 8D0DED000000 lea 0x000000ed
0000003E: 01D1 add ecx,edx
00000040: 81C1EDFFFFFF add ecx,eax,,0xFFFFFFED
00000046: FF31 push ecx,ecx
00000048: 8D0DD1000000 lea 0x000000d1
0000004E: 01D1 add ecx,edx
00000050: 81C1EDFFFFFF add ecx,eax,,0xFFFFFFED
00000056: 51 push ecx
00000057: E843000000 call 0x00000043
0000005C: B91C000000 mov ecx,0x0000001C
00000061: 51 push ecx
00000062: 8D0DD1000000 lea 0x000000d1
00000068: 01D1 add ecx,edx
0000006A: 81C1EDFFFFFF add ecx,eax,,0xFFFFFFED
00000070: 51 push ecx
00000071: E80C000000 call 0x0000000C
00000076: 5A pop edx
00000077: 5A pop edx
00000078: 58 pop eax
00000079: 59 pop ecx
...
def test_example_3():
    ...
    with open(os.path.join(TEST_FILE_DIR, 'positive/example3'), 'rb') as f:
        ...
        assert len(instructions) == len(EXAMPLE_3_OUTPUT) -2 
        # When outputs have labels, some lines will contain newlines in them. Join and re-split to get individual lines
        output = "\n".join(cheeseshredder.format.LabeledFormatter().print_instructions(in_order_parse)).splitlines()
